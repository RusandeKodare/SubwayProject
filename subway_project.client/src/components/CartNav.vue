...
const groupedSpecialsList = computed(() => {
  const map = {};
  const specials = orderStore.order.specials || [];
  for (const special of specials) {
    if (!map[special.name]) {
      map[special.name] = { ...special, quantity: 1 };
    } else {
      map[special.name].quantity += 1;
    }
  }
  return Object.values(map);
});
...
const isAddSubToCartDisabled = () => {
  const subHasBread = subStore.sub.products.findIndex(product => product.subCategoryId === 2) !== -1;
  if (!subHasBread) {
    return true;
  }
...
<template>
  ...
  <div class="cart">
    ...
    <div class="cart-header">
      ...
      <span
        v-if="orderStore.order.products.length > 0 || orderStore.order.subs.length > 0 || subStore.sub.products.length > 0 || orderStore.order.specials.length > 0">
        <div class="cart-footer">
          <button @click="checkout" :disabled="IsCheckoutDisabled()">Finalize order</button>
          <div v-if="IsCheckoutDisabled()">
            <p v-if="subStore.sub.products.length > 0">You cannot checkout while building your sub. Please add the sub
              to the cart, or clear the sub, to be able to checkout.</p>
    ...
    <!--Specials-->
    <div v-if="groupedSpecialsList.length > 0">
      <h3>Specials</h3>
    ...
    <div>
      <div v-for="sub in groupedCartSubList" class="cart-item-sub">
        ...
        <ul>
          <li v-for="product in sub.products" :key="product.id">{{ product.name }} — {{ product.quantity }} x {{
            product.price }} kr </li>
        </ul>
      </div>
    </div>
    <div v-if="orderStore.order.products.length !== 0 || orderStore.order.subs.length !== 0"
      class="cart-footer cancel-button">
